...
scores_file = dict({
	'actual': 'kotanietal2014_voa_actual_F-chunks_F-punct_T-samp_F-limit_T-save.csv',
...
def main():
	...
	main_dir = join(join("data", data_name), "results/2023_issc")
	# main_dir = join(join("data", data_name), "results/conf_report")

	# scores_dir = join(main_dir, system_map[script_name])
	# scores_path = join(scores_dir, scores_file[script_name])
	# scores_path = join(scores_dir, "voa_actual_mean.csv")
	# scores_path = join(main_dir, 
	# 	    "_".join(["formulae", data_name, script_name, "scores.csv"]))
	scores_path = join(main_dir, 
		    "_".join(["issc", data_name, script_name, "scores.csv"]))
	...
	if assess == "kotani":
		...
		# Kotani et. al., 2014
		feats = ['LEVEL','L_WORD', 'L_SENT', 'P_DIFF_WORDS', 'P_POLY_WORDS', 
	   			 'P_CONTRACTIONS', 'P_DEDUCTIONS', 'P_ELISIONS', 
				 'P_LINKINGS', 'P_REDUCTIONS', 'R_SPEECH']
		# feats = ['LEVEL', 
	   	# 		 'P_CONTRACTIONS', 'P_DEDUCTIONS', 'P_ELISIONS', 
		# 		 'P_LINKINGS', 'P_REDUCTIONS', 'R_SPEECH']
		
		scores_df = kotani_get_densities(scores_df)
		trimmed_df = scores_df[feats].loc[scores_df['LEVEL'] != 2]
		
		y = trimmed_df['LEVEL']
		X = trimmed_df.drop(columns='LEVEL')

		run_logistic_regression_repeated_k_fold(X, y, folds, repeats)

	if assess == "formulae":
		# Readability and listenability formulae
		...
		# accuracies = run_logistic_regression_x_features(trimmed_df, 1)
		# accuracies.extend(run_logistic_regression_x_features(trimmed_df, 2))
		# accuracies.extend(run_logistic_regression_x_features(trimmed_df, 3))
		# accuracies.extend(run_logistic_regression_x_features(trimmed_df, 4))
		# accuracies.extend(run_logistic_regression_x_features(trimmed_df, 5))
		# accuracies.extend(run_logistic_regression_x_features(trimmed_df, 6))
		
		# results_file = "_".join(["formulae", data_name, script_name, "lr-3-way.csv"])
		# results_path = join(main_dir, results_file)
		...
		curve_infos = run_learning_curve_x_features(trimmed_df, 4)
		# curve_infos.extend(run_learning_curve_x_features(trimmed_df, 2))
		# curve_infos.extend(run_learning_curve_x_features(trimmed_df, 3))
		# curve_infos.extend(run_learning_curve_x_features(trimmed_df, 4))
		...
		for curve_info in curve_infos:
			...
			plt.savefig(
				join(main_dir, 
					 "_".join(["lr-lrncrv", data_name, script_name, plot_id]) 
					 + ".png"), dpi=600)


	if assess == "content":
		# Content-based features
		feats = ['LEVEL', 'AVE_MINIW_COUNT', 'AVE_MONOSYLL_COUNT', 
	   			 'AVE_NOT_IN_DALE', 'AVE_SENT_LEN', 'AVE_WORD_LEN', 
				 'INV_AVE_SENT_LEN']
		scores_df = content_prepare(scores_df)

		# trimmed_df = scores_df.drop(columns='ID').loc[scores_df['LEVEL'] != 2]
		trimmed_df = scores_df[feats].loc[scores_df['LEVEL'] != 2]
		# trimmed_df = scores_df.drop(columns='ID')
		trimmed_df = scores_df[feats]
		...
		accuracies.extend(run_logistic_regression_x_features(trimmed_df, 5))
		accuracies.extend(run_logistic_regression_x_features(trimmed_df, 6))
		accuracies.extend(run_logistic_regression_x_features(trimmed_df, 7))
		accuracies.extend(run_logistic_regression_x_features(trimmed_df, 8))
		
		results_file = "_".join(["issc", data_name, script_name, "lr_ovr_content.csv"])
		results_path = join(main_dir, results_file)
		with open(results_path, mode = 'w') as results_f:
			writer = csv.writer(results_f)
			writer.writerow(["features", "mean", "stdev"])
